...
class HalfPlane(CSG3D):
	def __init__(self, zero, normal, *, eps = csg_eps):
		...
		super().__init__(netgen_csg.Plane(netgen_csg.Pnt(*zero), netgen_csg.Vec(*normal)), csg_boundaries = [
...
class Sphere(CSG3D):
	def __init__(self, center, radius, *, eps = csg_eps):
		eps *= radius
		super().__init__(netgen_csg.Sphere(netgen_csg.Pnt(*center), radius), csg_boundaries = [
			dolfin.CompiledSubDomain(
...
class Cylinder(CSG3D):
	def __init__(self, pointa, pointb, radius, *, eps = csg_eps):
		...
		eps *= radius
		super().__init__(netgen_csg.Cylinder(netgen_csg.Pnt(*pointa), netgen_csg.Pnt(*pointb), radius), csg_boundaries = [
			dolfin.CompiledSubDomain(
...
class OrthoBrick(CSG3D):
	def __init__(self, pointa, pointb, *, eps = csg_eps):
		...
		super().__init__(netgen_csg.OrthoBrick(netgen_csg.Pnt(*pointa), netgen_csg.Pnt(*pointb)), csg_boundaries = [
...
class CSG3DCollection(CSGCollection):
	...
	def add(self, csg3d, *, add_boundaries = True):
		...
		if add_boundaries:
			self.csg_boundaries.extend(csg3d.csg_boundaries)
...
class Layers(CSG3DCollection):
	def __init__(self, boundary, low, high, layers = 1, *, elements_per_layer = 2):
		super().__init__()
		self.csg_boundaries = boundary.csg_boundaries
		self.layers = layers
